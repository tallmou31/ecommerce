...
import { getEntitiesByIds } from '../product/product.reducer';
import { IUser } from 'app/shared/model/user.model';
import { getUser, getUserById } from 'app/modules/administration/user-management/user-management.reducer';
...
export const OrderDetail = () => {
  ...
  const user: IUser = useAppSelector(state => state.userManagement.user);
  ...
  useEffect(() => {
    if (orderEntity?.userId) {
      dispatch(getUserById(orderEntity.userId));
    }
  }, [orderEntity]);
  ...
  return (
    <div className="py-12 max-w-6xl mx-auto">
      ...
      <div>
        ...
        <p>
          <span className="font-bold">Nom Client : </span> <span>{user ? user.firstName + ' ' + user.lastName : orderEntity.userId}</span>
        </p>
        <p>
